...
class MusicDataset(torch.utils.data.Dataset):
    def __init__(self, csv_file, audio_dir, lyrics_dir, genre_map=None, tokenizer=None, max_len=130):
        ...
        def clean_name(name):
            ...
            name = re.sub(r"[^a-z0-9._]+", "_", name)  # keep dot for extension
            name = re.sub(r"_+", "_", name)
            return name.strip("_")
        ...
        for f in os.listdir(audio_dir):
            if f.lower().endswith(".mp3"):
                base = os.path.splitext(f)[0]  # remove extension
                key = self.clean_name(base)    # normalize
                self.audio_files[key] = os.path.join(audio_dir, f)
    ...
    def __getitem__(self, idx):
        ...
        if song_key not in self.audio_files:
            ...
            possible_matches = get_close_matches(song_key, self.audio_files.keys(), n=1, cutoff=0.6)
            if possible_matches:
                song_key = possible_matches[0]
                # print(f"⚠️ Closest match for '{row['song_id']}' is '{song_key}'")
            else:
                raise FileNotFoundError(
                    f"❌ Missing audio for: {song_key}\nAvailable keys: {list(self.audio_files.keys())[:20]}"
                )
        ...
        # Lyrics
        # --------------------------
        lyrics_filename = os.path.splitext(base_from_csv)[0] + ".txt"
        lyrics_path = os.path.join(self.lyrics_dir, lyrics_filename)
        if os.path.exists(lyrics_path):
